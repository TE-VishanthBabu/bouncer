...
public class DynamicRouter extends AbstractProcessor
{
  ...
  public static final String DEFAULT_RELATIONSHIPS = "StaticAnalysis,Error_Landing_Zone,"
	  		+ "HybridAnalysis,End_Zone,Sanitize";
  public static final String DEFAULT_PIPELINE = "StaticAnalysis,HybridAnalysis,"
  		+ "Sanitize,StaticAnalysis,HybridAnalysis,End_Zone";
  ...
  public static final PropertyDescriptor PIPELINE = new PropertyDescriptor.Builder()
      .name("pipeline")
   ...
   * @return none
   */
  private FlowFile populatePipeline(final ProcessSession session,
                                    ...
                                    throws SIAFTFatalProcessException
  {
    ...
    if (!StringUtils.isEmpty(entry))
    {
      ...
      {
      pipeline = pipelineMap.get(PIPELINE.getName());
      }
      else
      {
      ...
      flowfile = populatePipeline(session, flowfile, pipeline);
    }
    ...
    if (null == dest)
    {
      ...
      throw new SIAFTFatalProcessException("Pipeline [" + PIPELINE.getName()
