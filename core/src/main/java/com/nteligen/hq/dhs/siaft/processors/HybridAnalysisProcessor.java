...
public abstract class HybridAnalysisProcessor extends SIAFTFileProcessor
{
   ...
   * @param maecFlowFile The FlowFile where the maec report is to be written.
  ...
  protected void analyzeFile(ProcessSession session, ProcessContext context,
                             FlowFile flowFile, FlowFile reportFlowFile, FlowFile maecFlowFile)
                             throws AnalysisException
  {
    try (DropoffSession dropoff = getSessionFactory(context).getSession())
    {
      ...
      String jsonReportName = filename + ".json";
      String maecReportName = filename + ".xml";

      putFile(dropoff, session, flowFile, filename);
      getFile(dropoff, session, reportFlowFile, jsonReportName);
      ...
      getFile(dropoff, session, maecFlowFile, maecReportName);
      putReportAttributes(dropoff, session, maecFlowFile);

      session.getProvenanceReporter().modifyContent(maecFlowFile,
                                                    MODIFIED_REPORT_CONTENT);
      session.putAttribute(maecFlowFile, AnalysisDBWriter.ANALYSIS_SUCCESS,
                           Boolean.TRUE.toString());
    }
    ...
    throws ProcessException
  {
    ...
    FlowFile maecFlowFile = session.clone(flowFile, 0, 0);
    ...
    try
    {
      ...
      reportFlowFile = session.putAttribute(reportFlowFile, SIAFTUtils.ANALYSIS_ENGINE_ID,
                                            analyzeEngineId);
      maecFlowFile = session.putAttribute(maecFlowFile, SIAFTUtils.ANALYSIS_ENGINE_ID,
                                            analyzeEngineId);
    }
    catch (Exception ex)
    {
    ...
    try
    {
      analyzeFile(session, context, flowFile, reportFlowFile, maecFlowFile);
    }
    catch (Exception ex)
    {
      ...
      session.putAttribute(maecFlowFile, AnalysisDBWriter.ANALYSIS_SUCCESS,
          Boolean.FALSE.toString());
      session.putAttribute(maecFlowFile, AnalysisDBWriter.ANALYSIS_RESULTS,
          "An error occurred while analyzing the file");
    }

    onSuccess(session, flowFile, reportFlowFile, maecFlowFile);
  }
