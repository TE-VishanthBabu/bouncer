...
public class FiletypeSummaryView extends WebsiteView {
	@FunctionName("FiletypeSummaryView")
	public HttpResponseMessage run(@HttpTrigger(name = "req", methods = {
			HttpMethod.GET }, authLevel = AuthorizationLevel.ANONYMOUS, route = "website/views/{batchId}/filetypeSummaryView") HttpRequestMessage<Optional<String>> request,
			@BindingName("batchId") long batchId, final ExecutionContext context) {
		// get jwt from header
...
//		context.getLogger().info("Got user: " + user.getUserId());
		Batch batch;
		try {
			batch = BatchDAL.getBatch(batchId);
		...
		try {
			...
			JSONObject report = pgReport.buildFiletypeSummaryView();
